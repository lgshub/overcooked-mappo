...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, featurize_fn=1, start_order_list=None, cook_time=20, num_items_for_soup=3, delivery_reward=20, rew_shaping_params=None, layout_name="unnamed_layout"):
        """
    ...
    def get_state_transition(self, state, joint_action):
        ...
        #self.shaped_reward += self.calculate_distance_based_shaped_reward(state, new_state)

        shaped_reward += self.state_reward(state, new_state)
        # shaped_reward = self.shaped_reward

        return new_state, sparse_reward, shaped_reward
    
    
    def state_reward(self, state, new_state):
        rwd = 0
        idx = -1        
        num_items0 = 0
        num_items1 = 0
        for pot_pos in self.get_pot_locations():
            ...
            if state.has_object(pot_pos):
                ...
                soup_type, num_items0, cook_time = obj.state
            if new_state.has_object(pot_pos):
                obj = new_state.get_object(pot_pos)
                soup_type, num_items1, cook_time = obj.state
            # if num_items1>num_items0:    
            #     rwd += self.reward_shaping_params["PLACEMENT_IN_POT_REW"]*(1+2.8*(idx==1))
        #hint needed: cramped; 
        self.zone = [(3,1),(3,2),(3,3),(3,4),(3,5),(3,6)]
        idx = -1
        for player0, player1 in zip(state.players, new_state.players):
            idx += 1            
            
            if (not player0.has_object()) and player1.has_object():
                obj = player1.get_object()                
                #if idx==1:
                #rwd += self.reward_shaping_params["ONION_PICKUP_REW"]*(obj.name[0]=='o')
                #if idx==0:
                #rwd += self.reward_shaping_params["DISH_PICKUP_REW"]*(obj.name[0]=='d')
            elif player0.has_object() and player1.has_object():
                obj0 = player0.get_object()
                obj1 = player1.get_object()
                # 让左侧更倾向于送
                #if idx==0:   
                #  rwd += self.reward_shaping_params["SOUP_PICKUP_REW"]*(obj0.name[0]=='d' and obj1.name[0]=='s')
                #else:
                #  rwd += self.reward_shaping_params["SOUP_PICKUP_REW"]*(obj0.name[0]=='d' and obj1.name[0]=='s')
        players = new_state.players
        # if len(state.player_objects_by_type['soup'])>0:
        #     rwd += 3
        return rwd
    ...
    def resolve_interacts(self, new_state, joint_action):
        ...
        idx = -1
        self.counter_list = [(2,2),(3,2),(4,2),(5,2)]
        zone1 = [(2,1),(3,1),(4,1),(5,1)]
        zone2 = [(2,3),(3,3),(4,3),(5,3)]
        for player, action in zip(new_state.players, joint_action):
            ...
            #随手乱放需要管！
                ...
                shaped_reward += self.reward_shaping_params["ONION_PICKUP_REW"]
                ...
                if player.get_object().name == 'dish' and new_state.has_object(i_pos):
                    ...
                    if num_items == self.num_items_for_soup and cook_time >= self.soup_cooking_time:
                        ...
                        #print("soup picked:")
                        self.pickup_times += 1
                        #print(self.pickup_times)
                        if len(ready_pots)<2 or self.layout_name!="asymmetric":
                          shaped_reward += self.reward_shaping_params["SOUP_PICKUP_REW"]
                    ...
                    if not new_state.has_object(i_pos):
                        ...
                        #if idx==1:
                        if len(cooking_pots)>0 or self.layout_name!="asymmetric":
                ...
                if obj.name == 'soup':
                    ...
                    #if len(cooking_pots)>0:
                    shaped_reward += self.reward_shaping_params["DELIVERY_REW"]
                    #print("send:")
                    self.send_times += 1
    ...
    def lossless_state_encoding(self, overcooked_state):
        ...
        map = self.terrain_mtx
        code = []
        def class_of_object(state_obj):
            if state_obj.name[0]=='o':
                return [1,0,0]
            elif state_obj.name[0]=='d':
                return [0,1,0]
            elif state_obj.name[0]=='s':
                return [0,0,1]
            else:
                b=1
        self.counter_list = []
        if self.layout_name=="circuit":
            self.counter_list = [(2,2),(3,2),(4,2),(5,2)] #circuit
        elif self.layout_name=="forced":
            self.counter_list = [(2,1),(2,2),(2,3)] #forced
        seats = []
        for j in range(self.width):
            for i in range(self.height):
                b = map[i][j]
                if b[0]=='P':
                    if overcooked_state.has_object((j,i)):
                        obj = overcooked_state.get_object((j, i))
                        state = obj.state
                        code.append(state[1]>=self.num_items_for_soup) #num_item
                        code.append(state[2]>=self.soup_cooking_time) #cooking_time
                    else:
                        code.append(0) 
                        code.append(0)
                elif b[0]=='X' and (j, i) in self.counter_list:
                  if overcooked_state.has_object((j, i)):
                        obj = overcooked_state.get_object((j, i))
                        code.append(obj.name[0]=='o')
                        code.append(obj.name[0]=='d')
                        code.append(obj.name[0]=='s')
                  else:
                        code.append(0)
                        code.append(0)
                        code.append(0)
        ...
        for i, player in enumerate(overcooked_state.players):
                ...
                code.append(player_orientation_idx%2)
                code.append(player_orientation_idx//2)
                #code = np.concatenate((code,list(player.position))).tolist()
                code = np.concatenate((code,[player.position==pos for pos in seats])).tolist()
                if player.has_object():
                    obj = player.held_object
                    code=np.concatenate((code,class_of_object(obj))).tolist()
                else:
                    code=np.concatenate((code,[0,0,0])).tolist()
        code = np.array([code])
        return [code, code]
